...
> **[2026-08-24 撤回済み]** このディレクトリの方式(JSON出力 → GAS(`gas/Code.gs`)が
> pull型でfetchして既存スプレッドシートへ書き込む)は、**廃止・撤回されました**。
> 理由: GASという中継を挟まず、作業ルーム(Claude)自身が新規のGoogleスプレッドシート
> (またはExcelファイル)を作成して直接書き込む、よりシンプルな方式に統一したため
> (`gurii-gabreh/progress-tracker-dashboard` の依頼タスクタブ TEX-001/TEX-002 を参照)。
> 新方式では、認証情報(`credentials/service_account.json`・`config.json`)をチャットへ
> 貼り付ける必要も無くなっている。
> 以下は経緯の記録として残しているが、**今後の実装・運用の参照先としては使わないこと**。
